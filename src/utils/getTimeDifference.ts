export function getTimeDifference(datetimeString: any) {
    const [first, second] = datetimeString.split("T")
    const newSecond = second.replace("Z", "")
    const third = `${first} ${newSecond}`
    const before: any = new Date(third)
